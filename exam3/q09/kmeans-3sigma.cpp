...
bool UpdateClusters()
{
    ...
    // Phase III
    // Evict any data point more than 4 sigmas away
    // from the mean of its currently assigned cluster
    if (converged)
    {
        for (auto c : *clusters)
        {
            // Calculate the standard deviation of distance from
            // each cluster's center to each of its assigned points
            c->dist_sigmas = 0;
            int count = 0;
            for (auto dp : *dataPoints)
            {
                if (dp->c == c)
                {
                    c->dist_sigmas += sqrt(pow(dp->x - c->x, 2) +
                                           pow(dp->y - c->y, 2));
                    count++;
                }
            }
            c->dist_sigmas = sqrt(c->dist_sigmas / count) * 3;

            // For each data point belonging to this cluster,
            // check if its distance is > 4 sigmas from the center
            for (auto dp : *dataPoints)
            {
                if (dp->c == c)
                {
                    double dist = sqrt(pow(dp->x - c->x, 2) +
                                       pow(dp->y - c->y, 2));
                    if (dist > c->dist_sigmas)
                    {
                        // We need to evict this data point from its cluster
                        // so the algorithm clearly has not yet converged
                        converged = false;
                        dp->evictionCount++;
                        // If we have now evicted this data point from all possible clusters,
                        // then we should remove this element from the vector of data points
                        // provided we are not creating an empty cluster
                        if (dp->evictionCount == clusters->size())
                        {
                            if (c->population > 1)
                            {
                                c->population--;
                                cout << "Data point at"
                                     << " (" << dp->x << ", " << dp->y << ")"
                                     << " was evicted" << endl;
                                // Find and remove this element in the vector of data points
                                auto it = std::find((*dataPoints).begin(), (*dataPoints).end(), dp);
